...
class Booking extends Entry {
  ...
  def isBookingExist(
    ...
    bookingTable.filter(
      each => each.selectedDate === selectedDate && each.serviceCenter === serviceCenter && each.serviceType === serviceType && each.timeSlot === timeSlot
    ).result.headOption.map {
      case Some(booking) => booking.id
      case None => -1
    }
  }

  def createBookingIfNotExist(
                               selectedDate: String,
                               serviceCenter: String,
                               serviceType: String,
                               userName: String,
                               ...
                               timeSlot: String,
                               createdDateTime: LocalDateTime
                             ): Future[Either[BookingRow,Int]] = {
    isBookingExist(selectedDate, serviceCenter, serviceType, timeSlot)
      .flatMap(
      bookingId => if (bookingId == -1) {
        create(
          selectedDate,
          serviceCenter,
          serviceType,
          ...
          createdData => Left(createdData)
        )
      } else {
        Future.successful(Right(bookingId))
      }
    )
  }
